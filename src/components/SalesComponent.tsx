...
import { ArrowRightIcon, ChartPieIcon, ViewColumnsIcon } from '@heroicons/react/20/solid'
...
const SalesComponent = () => {
  ...
  return (
    <Card className="max-w-full mx-auto">
        <Flex className="space-x-8 flex-col lg:flex-row">
            ...
            <TabGroup index={selectedIndex} onIndexChange={setSelectedIndex}>
                <TabList variant="solid">
                    <Tab icon={ChartPieIcon} style={{color: `${selectedIndex === 0 ? 'blue' : ''}`}}>Chart</Tab>
                    <Tab icon={ViewColumnsIcon} style={{color: `${selectedIndex === 1 ? 'blue' : ''}`}}>List</Tab>
                </TabList>
            </TabGroup>
        </Flex>
        ...
        {selectedIndex === 0 ? (
            <DonutChart data={stocks} valueFormatter={dataFormatter} showAnimation={false} category='value' index='name' className='mt-6'/>
        ) : (<>
